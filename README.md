...
- `POST /api/auth/register`（相容舊流程，已非主要入口）
- `POST /api/auth/login`（相容舊流程，已非主要入口）
- `PATCH /api/auth/password`（相容舊流程）
- `GET /api/pets`
- `GET /api/pets/:id`
- `GET /api/stories`
- `GET /api/shelters`
...
如果你想灌入一組可展示的模擬資料，包含毛孩照片與幾筆收養申請，可以跑：

```bash
npm run seed:demo
```

這會把 demo 資料寫進本機 D1，方便你在 `/pets` 和 `/admin/adoptions` 直接看到效果。現在會有 7 隻示範毛孩和 7 筆示範收養申請。

## Google 登入設定

目前正式會員登入改為 Google OAuth，不需要再寄驗證信。

正式站至少要設定：

- `GOOGLE_CLIENT_ID`
- `GOOGLE_CLIENT_SECRET`
- Google Cloud Console OAuth 2.0 redirect URI：`https://你的網域/api/auth/google/callback`

這兩個值建議放在 Cloudflare Pages 專案的 `Settings > Variables and Secrets`，正式環境與 Preview 環境分開設定。

本機開發如果要串 Google 登入，也要在 OAuth client 裡補上對應的 localhost redirect URI，例如：

- `http://localhost:4321/api/auth/google/callback`

如果你要本機模擬 Pages Functions，可以用 `wrangler pages dev` 並傳入對應的 `GOOGLE_CLIENT_ID` 與 `GOOGLE_CLIENT_SECRET`。

如果你還保留舊的 Email 註冊流程，`/api/auth/register` 仍會依賴 MailChannels 與寄件網域驗證，但它已不是主要登入路徑。
